  ...
  return async (input) => {
    ...
    const result = await generateObject({
      ...
      system: [
        ...
        "If the PR adds a newly documented component or primitive, assume the docs change set should include the primary MDX page plus any required companion docs integration updates in the existing docs app.",
        "Infer the appropriate companion files from the current repo patterns instead of inventing a one-off structure.",
        "Typical companion updates include: creating or updating a supporting React docs demo component, exporting it from the existing docs barrel files, registering those components in the MDX component map, adding the new page slug to docs navigation metadata, and updating homepage summary counts when they reflect the total documented components.",
        "When updating existing integration files, preserve their structure and formatting, make the smallest targeted change, append or insert new entries in the correct existing location, and avoid rewriting unrelated content.",
        "Only change files that are directly required to make the new or updated documentation page work correctly in this repo. Do not touch unrelated docs pages or unrelated application files.",
      ].join(" "),
      prompt: [
        ...
        [
          "- Create or update the supporting docs React component file when the MDX page embeds custom demos or API tables",
          "- Update the existing docs barrel export files so new demo components remain importable from the shared docs entrypoint",
          "- Update /apps/fumadocs/src/components/mdx.tsx so any newly referenced MDX components are registered in the component map",
          "- Update /apps/fumadocs/content/docs/meta.json by appending the new slug in the correct existing location instead of rewriting the list from scratch",
          "- Update /apps/fumadocs/src/app/(home)/page.tsx by adjusting the displayed component count in place when the homepage total should reflect the newly documented component",
          "- Preserve unrelated entries, ordering patterns, structure, and formatting in existing files",
          "- Do not modify unrelated MDX pages or unrelated application files",
        ].join("\n"),
      ].join("\n\n"),
    });
